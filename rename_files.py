...
def get_files_in_folder(path: str = '') -> List[str]:
    FOLDER_CONTENT: Final[List[str]] = get_folder_content(path)
...
def rename_file(file_path: str = '', pattern: str = '', new_name: str = '') -> None:
    ...
    if is_possible_to_rename:
        NEW_NAME: Final[str] = file_name.replace(pattern, new_name, 1)
        NEW_PATH: Final[str] = os.path.join(path, NEW_NAME)
        os.rename(file_path, NEW_PATH)


def rename_files(path: str = '', pattern: str = '', new_name: str = '') -> None:
    FILES: Final[List] = get_files_in_folder(path)
...
def main(argv: List[str] = []) -> None:
    ...
    rename_files(path, pattern, name)
